...
Running notes for whoever maintains this: which account owns what, how to deploy,
how to read the logs. The project overview lives in [../README.md](../README.md).
...
| **Neon** | Berkeley | The Postgres behind the task API — `us-east-1`, host `<neon-host>` | `DATABASE_URL` in Vercel points at that host |
