...
- **Three separate accounts own this system:** GitHub `slabarcaf`, Vercel and Google Cloud under the Berkeley address, Oracle for the VM. Signing in to one is not enough, and the Vercel↔GitHub link is cross-account. The table in `README.md` says which holds what — read it before assuming you have access.
