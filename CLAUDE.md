...
- **Everything is the Berkeley Google account except GitHub, which is `slabarcaf`.** Vercel, Google Cloud and Neon are all Berkeley; the VM is Oracle, reached with `~/.ssh/id_ed25519`. So the Vercel↔GitHub deploy link is cross-account, and signing in to one identity is not enough. The table in `README.md` says which holds what — read it before assuming you have access.
